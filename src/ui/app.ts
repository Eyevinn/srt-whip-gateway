...
async function updateTransmitters() {
  ...
  transmitters.forEach((tx) => {
    ...
    element.innerHTML =
      `<span class="srtPort">${tx.port}</span><br>` +
      `<a class="whipUrl" href="${tx.whipUrl}">WHIP</a>` +
      (tx.passThroughUrl
        ? ` <a class="srtUrl" href="${tx.passThroughUrl}">SRT</a>`
        : '') +
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const addTransmitterButton =
    document.querySelector<HTMLButtonElement>('#addTransmitter');
  addTransmitterButton.addEventListener('click', async () => {
    const srtPort =
      document.querySelector<HTMLInputElement>('#newSrtPort').value;
    const whipUrl =
      document.querySelector<HTMLInputElement>('#newWhipUrl').value;
    const restreamUrl =
      document.querySelector<HTMLInputElement>('#restreamUrl').value;
